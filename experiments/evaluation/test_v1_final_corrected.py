#!/usr/bin/env python3
"""
DRAI V1 Final Evaluation - CORRECTED VERSION

This script runs the definitive test of DRAI V1 on the simple stories task,
using the correct evaluation methodology from simple_evaluator.py.

Expected results:
- Baseline: ~93% accuracy
- Phase 2 DRAI: ~13% accuracy (catastrophic failure)
- V1 DRAI: ~88-93% accuracy (SUCCESS!)

Fixes from initial version:
1. Greedy sampling (do_sample=False) instead of random sampling
2. Substring/fuzzy answer matching instead of exact-only matching
3. Take first sentence, not just first word
"""
...
def score_answer(predicted: str, correct: str) -> bool:
    """
    Score answer with flexible matching (from simple_evaluator.py).
...
def evaluate_model(model, tokenizer, stories, model_name="Model"):
    ...
    for i, story in enumerate(stories, 1):
        ...
        for q_idx, question in enumerate(story.questions):
            ...
            # Create prompt (using simple_evaluator format)
...
Answer:"""
            ...
            # Take first sentence (not just first word!)
...
def main():
    print("\n" + "="*70)
    print("DRAI V1 FINAL EVALUATION (CORRECTED)")
    print("="*70)
    ...
    # Save results
    results_file = Path(__file__).parent / "results" / "v1_final" / "comparison_corrected.json"
